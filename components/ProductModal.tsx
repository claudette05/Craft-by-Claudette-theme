...
const ProductModal: React.FC<ProductModalProps> = ({ product, onClose, onAddToCart, onViewDetails }) => {
    ...
    return (
        ...
        >
            ...
            />
            <motion.div
                className="relative w-full max-w-4xl bg-pink-50 rounded-xl shadow-lg flex flex-col md:flex-row max-h-[90vh]"
                variants={modalVariants}
                onClick={e => e.stopPropagation()}
            >
                <button
                    onClick={onClose}
                    className="absolute top-3 right-3 z-10 p-1 rounded-full text-zinc-500 hover:bg-pink-100 transition-colors"
                ...
                <div className="w-full md:w-1/2 p-6 flex flex-col justify-center overflow-y-auto">
                    ...
                    {hasSale ? (
                        <div className="flex items-baseline space-x-3 mb-4">
                            ...
                            <p className="text-lg font-light text-gray-500 line-through">GH₵{product.price.toFixed(2)}</p>
                        </div>
                    ) : (
                        <p className="text-2xl font-light text-amber-600 mb-4">GH₵{product.price.toFixed(2)}</p>
                    )}

                    <p className="text-gray-600 mb-6 leading-relaxed text-sm h-24 overflow-y-auto">{product.description}</p>
                    ...
                    <button 
                        ...
                        className="mt-3 text-center text-sm font-medium text-amber-600 hover:text-amber-500 hover:underline"
                    >
                        View Full Details
                    </button>
                </div>
